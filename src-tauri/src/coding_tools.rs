...
fn validate_git_remote_name(remote: &str) -> Result<(), ProviderError> {
    let r = remote.trim();
    if r.is_empty() {
        return Err(tool_err("git remote name is empty"));
    }
    if r.starts_with('-')
        || !r
            .chars()
    ...
    {
        return Err(tool_err(
            "git remote must be a configured remote name such as `origin`",
        ));
    }
    Ok(())
}

fn validate_remote_urls_for_github_auth(
    repo_dir: &Path,
    remote: &str,
    push_urls: bool,
) -> Result<(), ProviderError> {
    ...
    let out = cmd
        .output()
        .map_err(|e| tool_err(format!("git remote validation failed: {e}")))?;
    if !out.status.success() {
        return Err(tool_err(format!(
            "git remote validation failed (exit {}): {}",
    ...
    if !saw_url {
        return Err(tool_err(format!("git remote `{remote}` has no URL")));
    }
    Ok(())
}
...
pub async fn run_coding_tool(
    ...
    match name {
        ...
        "coding_git_push" => {
            ...
            validate_remote_urls_for_github_auth(&repo_dir, remote, true)?;
            let branch = v["branch"]
                .as_str()
                .map(str::trim)
                .filter(|s| !s.is_empty());
        ...
        "coding_git_pull" => {
            ...
            let pat = crate::git_auth::require_github_pat(settings)?;
            let remote = v["remote"].as_str().unwrap_or("origin").trim();
            validate_remote_urls_for_github_auth(&repo_dir, remote, false)?;
            let branch = v["branch"]
                .as_str()
                .map(str::trim)
                .filter(|s| !s.is_empty());
...
mod tests {
    ...
    #[test]
    fn validate_git_remote_name_rejects_urls_and_options() {
        assert!(validate_git_remote_name("origin").is_ok());
        assert!(validate_git_remote_name("upstream.main").is_ok());
        assert!(validate_git_remote_name("https://example.com/repo.git").is_err());
        assert!(validate_git_remote_name("--upload-pack=sh").is_err());
